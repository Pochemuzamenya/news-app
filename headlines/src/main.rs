...
fn render_footer(ctx: &CtxRef) {
    TopBottomPanel::bottom("footer").show(ctx, |ui| {
        ui.vertical_centered(|ui| {
            ui.add_space(10.);
            //add the api source website info
            ui.add(Label::new("API souce: newsapi.org").monospace());
            //then we'll add a link to the egui framework
            ui.add(
                Hyperlink::new("https://github.com/emilk/egui")
                    .text("Made with egui")
                    .text_style(TextStyle::Monospace),
            );
            //then we'll put the github link to the headlines source code
            ui.add(
                Hyperlink::new("https://github.com")
                    .text("adnrey/filatov")
                    .text_style(TextStyle::Monospace),
            );
            ui.add_space(10.);
